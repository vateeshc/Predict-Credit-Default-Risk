...
```{r}
splits_h2o <- h2o.splitFrame(data_h2o, ratios = c(0.8,0.1), seed = 1234)
...
te_cols<- c("OCCUPATION_TYPE","ORGANIZATION_TYPE")
...
```{r}
y <- "TARGET"
x <- setdiff(names(train_h2o), c(y, te_cols, "SK_ID_CURR"))
...
```{r}
ext_test_h2o <- h2o.target_encode_apply(test_h2o, x = as.list(te_cols), y = "TARGET",
                                     target_encode_map = te_map, 
                                     holdout_type = "None",
                                      noise_level = 0,
                                     seed = 1234
...
head(ext_test_h2o[c("OCCUPATION_TYPE", "TargetEncode_OCCUPATION_TYPE")])
```


```{r}
performance_h2o <- h2o.performance(model = , newdata = ext_test_h2o)
```
...
head(ext_newdata_h2o[c("OCCUPATION_TYPE", "TargetEncode_OCCUPATION_TYPE")])

```

```{r}
prediction_h2o <- h2o.predict(h2o.getModel(model_id="DRF_0_AutoML_20180831_130113"), newdata = as.h2o(x_test_processed_tbl)) 
...
```{r}
Model_1 <- h2o.getModel("DRF_0_AutoML_20180831_130113")
varimp_h2o<-h2o.varimp(Model_1)
var_top40<-varimp_h2o%>%select(variable,scaled_importance)%>%arrange(desc(scaled_importance))%>%top_n(40)%>%select(variable)
h2o.varimp_plot(model=h2o.getModel("DRF_0_AutoML_20180831_130113"),num_of_features = 40)
var_top40
```

```{r}
ggplot_tbl<-(select(bind_cols(x_train_processed_tbl,y_train_processed_tbl),c("TARGET","SK_ID_CURR", var_top40$variable)))
head(ggplot_tbl)

var <- as.character(var_top40$variable)

for (i in 1:seq_along(names(ggplot_tbl))){
  if(names(ggplot_tbl[i]) %in% c(var) ){
g <- ggplot(ggplot_tbl,aes_string(names(ggplot_tbl[i]))) 
g + geom_density(aes(color=TARGET))
print(g)
}
}
```